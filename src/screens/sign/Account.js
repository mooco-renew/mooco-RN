...
export default function Account({ route }) {
  ...
  useEffect(() => {
    if (
      ...
      if (email != "" && !secondTrue(auth))
        setErrorMessage("이메일 인증을 완료해주세요.");
      else if (pw != "" && !validatePassword(pw))
        setErrorMessage(
          "알파벳, 숫자, 특수문자를 포함하여 8자리 이상 작성해주세요."
        );
      else if (pw != "" && email != "" && !firstTrue(auth))
        setErrorMessage("이용약관에 동의해주세요.");
      setIsAvail(false);
    }
    if (auth[1] === true) {
      // 이메일 인증 후 돌아올 시, 인증 화면 닫기
      setIsShow(false);
    }
  }, [id, pw, auth]);
    ...
    <View style={styles.container}>
      {!isShow && (
        <View style={styles.container}>
          <View style={styles.inputbox}>
            ...
            <View style={styles.authcontainer}>
              ...
              <TouchableOpacity
                style={[
                  styles.emailbutton,
                  (!validateEmail(email) || secondTrue(auth)) &&
                    styles.buttondisable,
                ]}
                onPress={() => sendEmail()}
                disabled={!validateEmail(email) || secondTrue(auth)}
              >
                <Text style={styles.emailbuttontext}>
                  {secondTrue(auth) ? "완료" : "인증"}
                </Text>
              </TouchableOpacity>
            </View>
          ...
          <TouchableOpacity
            style={[styles.button, !isAvail && styles.buttondisable]}
            onPress={() => cilckAccount()}
            disabled={!isAvail}
          >
          ...
          <View style={styles.checkcontainer}>
            <Checkbox
              style={[
                styles.checkbox,
                secondTrue(auth) && styles.checkboxdisble,
              ]}
              value={firstTrue(auth)}
              onValueChange={() =>
                navigation.navigate("Agreement", { id: id, pw: pw })
              }
              color={firstTrue(auth) ? "#000000" : undefined}
              disabled={firstTrue(auth)}
            />
            <Text style={styles.checktext}>
              가입과 동시에 이용 약관 및 개인정보 처리방침에 동의합니다.
            </Text>
          </View>
          ...
          <View style={styles.helptextbox}>
            <TouchableOpacity onPress={() => navigation.navigate("Login")}>
              <Text style={styles.helptext}>로그인</Text>
            </TouchableOpacity>
          </View>
        </View>
      )}
      {isShow && (
        <EmailAuth email={email} setShowAuth={setAuth} setCode={setCode} />
      )}
    </View>
  );
}
